...
export class ApprovalService {
  ...
  static async createRequest(input: CreateApprovalInput): Promise<ApprovalRequestRecord> {
    try {
      ...
      console.warn('ApprovalService.createRequest fallback:', err.message);
      return {
        id: `appr_fallback_${Date.now()}`,
        agentType: input.agentType,
        actionType: input.actionType,
        requestedAmount: input.requestedAmount,
        policyLimit: input.policyLimit,
  ...
  static async getPendingRequests(): Promise<ApprovalRequestRecord[]> {
    try {
      ...
      console.warn('ApprovalService.getPendingRequests read error fallback:', err.message);
      return [];
    }
  }
  ...
  static async decideRequest(
    ...
    try {
      ...
      console.warn('ApprovalService.decideRequest fallback:', err.message);
      return {
        id: requestId,
        ...
        policyLimit: 1000,
        reason: 'Manually updated decision.',
        payloadJson: '{}',
        status: decision,
        decidedBy,
